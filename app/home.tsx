...
function SlotCard({
  meta, onPress, onDelete, index,
}: {
  meta: SlotMeta; onPress: () => void; onDelete: () => void; index: number;
}) {
  const { C } = useTheme();
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.timing(fadeAnim,  { toValue: 1, duration: 380, delay: index * 90, useNativeDriver: true }),
      Animated.timing(slideAnim, { toValue: 0, duration: 380, delay: index * 90, useNativeDriver: true }),
    ]).start();
  }, []);
        ...
        <View style={s.slotMiddle}>
          {occupied ? (
            <>
              ...
              <Text style={s.slotMeta}>{`Year ${meta.year} · Week ${meta.week} · Prestige ${meta.prestige ?? 0}`}</Text>
              {meta.lastSaved && (
                <Text style={s.slotDate}>{`Saved ${formatDate(meta.lastSaved)}`}</Text>
              )}
            </>
          ) : (
            <>
        ...
        <View style={s.slotRight}>
          {occupied ? (
            <View style={s.slotActions}>
              ...
              <TouchableOpacity style={s.deleteBtn} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} onPress={onDelete}>
...
export default function HomeScreen() {
  ...
  function handleDelete(meta: SlotMeta) {
    if (Platform.OS === 'web') {
      ...
      if (confirmed) deleteSave(meta.slot).then(fetchSlots);
    } else {
      Alert.alert(
        'Delete Save',
        ...
        [
          ...
          { text: 'Delete', style: 'destructive', onPress: async () => { await deleteSave(meta.slot); fetchSlots(); } },
        ],
      );
    }
  }
            ...
            <View style={s.slotsList}>
              {slots.map((meta: SlotMeta, i: number) => (
                <SlotCard
                  ...
                  meta={meta}
                  index={i}
                  onPress={() => { if (loadingSlot) return; handleSlotPress(meta); }}
...
function makeStyles(C: ReturnType<typeof useTheme>['C']) {
  return StyleSheet.create({
    ...
    filmLine: { position: 'absolute', top: 0, bottom: 0, left: 28, width: 1, backgroundColor: C.borderGold },
    ...
    titleRule:  { width: 60, height: 1, backgroundColor: C.borderGold, marginTop: 18, marginBottom: 14 },
    ...
    slotCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 10, borderWidth: 1, paddingVertical: 16, paddingHorizontal: 16, overflow: 'hidden' },
    slotCardOccupied: { backgroundColor: C.cardBg, borderColor: C.borderGold },
    slotCardEmpty:    { backgroundColor: 'transparent', borderColor: C.border, borderStyle: 'dashed' },
