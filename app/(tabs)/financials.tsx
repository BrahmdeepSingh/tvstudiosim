...
import {
  View, Text, ScrollView, TouchableOpacity, StyleSheet, Alert, Image, Platform, Switch } from 'react-native';
...
function FilmRibbonAmbient({ color }: { color: string }) {
  return (
    <Image
      source={require('../../assets/tvbg.png')}
      style={[StyleSheet.absoluteFill, { tintColor: color, opacity: 0.06 }]}
      resizeMode="repeat"
      pointerEvents="none"
    />
  );
  ...
  return (
    <View>
      <View style={{ flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
        <Text style={{ color: C.text, fontFamily: 'BebasNeue_400Regular', fontSize: 44, letterSpacing: 1 }}>{value}</Text>
        <Text style={{ color: barColor, fontFamily: 'Manrope_700Bold', fontSize: 14 }}>{label}</Text>
      </View>
      <View style={{ height: 8, backgroundColor: C.border, borderRadius: 4, overflow: 'hidden', marginBottom: 10 }}>
        <View style={{ height: '100%', width: `${pct}%` as any, backgroundColor: barColor, borderRadius: 4 }} />
      </View>
      <Text style={{ color: C.muted, fontFamily: 'Manrope_400Regular', fontSize: 12, lineHeight: 18, marginTop: 4 }}>
...
function StatRow({ label, value, color }: { label: string; value: string; color?: string }) {
  ...
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 11 }}>
      <Text style={{ color: C.muted, fontFamily: 'Manrope_400Regular', fontSize: 14 }}>{label}</Text>
      <Text style={{ color: color ?? C.text, fontFamily: 'Manrope_600SemiBold', fontSize: 14 }}>{value}</Text>
    </View>
  );
}
...
export default function StudioScreen() {
  ...
  return (
    <SafeAreaView edges={['top']} style={s.container}>
      ...
      />
      <FilmRibbonAmbient color={C.gold} />
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.actionsCard}>
          ...
          <View style={s.actionRow}>
            <View>
              <Text style={s.actionLabel}>Light Mode</Text>
              <Text style={s.actionSub}>Switch between dark and light theme</Text>
            </View>
            <Switch
              value={!isDark}
              onValueChange={toggle}
              trackColor={{ false: C.border, true: C.goldMid }}
              thumbColor={isDark ? C.muted : C.gold}
            />
          </View>
          <View style={s.divider} />
...
function makeStyles(C: ReturnType<typeof useTheme>['C']) {
  return StyleSheet.create({
    ...
    allStudiosBtn:     { backgroundColor: C.goldDim, borderWidth: 1, borderColor: C.borderGold, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
