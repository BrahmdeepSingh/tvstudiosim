...
function qualityLabel(q: number, amber: string, red: string): { label: string; color: string } {
  if (q >= 80) return { label: 'Outstanding', color: '#2d8a5e' };
  if (q >= 65) return { label: 'Strong',       color: '#5a9e45' };
  if (q >= 50) return { label: 'Solid',         color: amber };
...
export default function MarketingScreen() {
  ...
  const ql = qualityLabel(season.qualityScore, C.amber, C.red);
        ...
        {airDateSet && !isEditing ? (
          ...
          <View style={s.airDateCard}>
            ...
            {(() => {
              ...
              const color = win.type === 'cultural' ? C.gold : C.teal;
              return (
...
function makeStyles(C: ReturnType<typeof useTheme>['C']) {
  return StyleSheet.create({
    ...
    winSub:           { fontFamily: 'Manrope_400Regular', color: C.muted, fontSize: 11, marginTop: 2 },
    noWinRow:         { marginBottom: 12, paddingVertical: 4 },
    noWinText:        { fontFamily: 'Manrope_400Regular', color: C.mutedMid, fontSize: 11, textAlign: 'center' },
    ...
    lockedBadge:          { backgroundColor: C.muted + '18', borderWidth: 1, borderColor: C.muted + '40', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 },
    lockedBadgeText:      { color: C.muted, fontFamily: 'Manrope_700Bold', fontSize: 10 },
