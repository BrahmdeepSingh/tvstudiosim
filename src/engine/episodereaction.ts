...
const PERSONAS: Record<string, PersonaConfig> = {
  ...
  stan: {
    ...
    midHigh: [
      ...
      (s, ep) => `${s} episode ${ep}: a perfectly pleasant way to spend an hour. that's it. that's the tweet.`,
      ...
      (s, ep) => `${s} episode ${ep}: comfortable, consistent, capable. I show up every week for a reason.`,
      ...
      (s, ep) => `${s} ep ${ep}: the episode where nothing went wrong and something occasionally went right.`,
    ...
    trendUp: [
      ...
      (s, ep) => `${s} ep ${ep}: the show remembered what made me fall for it. I appreciate that. genuinely.`,
    ...
    trendDown: [
      ...
      (s, ep) => `${s} ep ${ep}: a minor disappointment after a really good stretch. I'll allow it. once.`,
  ...
  critic: {
    ...
    high: [
      ...
      (s, ep)          => `${s} episode ${ep}: the season's best hour so far. The show is operating at a level few manage.`,
      ...
      (s, ep)          => `${s} episode ${ep}: this is why television is worth taking seriously. A complete hour of work.`,
      ...
      (s, ep)          => `${s} episode ${ep}: formally ambitious, emotionally grounded. Both things. Simultaneously.`,
    ...
    midHigh: [
      ...
      (s, ep) => `${s} episode ${ep}: a mid-season hour doing mid-season work. The show is in control.`,
      (s, ep) => `Episode ${ep} of ${s} is efficient. That's praise. The show doesn't waste what it has.`,
      (s, ep) => `${s} episode ${ep}: nothing surprising here. The show knows its lane and stays in it. That's not nothing.`,
      (s, ep) => `This is what a well-run series looks like in its middle episodes. ${s} ep ${ep} is a reliable hour.`,
      (s, ep) => `${s} ep ${ep}: sturdy craft in service of a story that earns modest rewards. Fine television.`,
      (s, ep) => `Episode ${ep} of ${s} advances the season without stumbling. Workmanlike but not lazy.`,
      (s, ep) => `${s} episode ${ep}: three strong scenes and a serviceable connective tissue. The show is consistent.`,
      (s, ep) => `The performances are carrying ${s} episode ${ep} more than the script, but the script holds up.`,
      (s, ep) => `${s} ep ${ep}: the narrative legwork episode. Not exciting. Necessary. Well-executed.`,
      ...
      (s, ep) => `${s} ep ${ep}: not a landmark hour. A quiet, well-made one. Some weeks that's the show.`,
      ...
      (s, ep) => `${s} ep ${ep}: every piece doing exactly what it should. No more, no less. Good television.`,
    ],
    midLow: [
      ...
      (s, ep) => `${s} episode ${ep}: the writing is doing fine work around a story beat that isn't pulling its weight.`,
      (s, ep) => `Episode ${ep} of ${s} has the moves but not the conviction. Technical success, dramatic near-miss.`,
      (s, ep) => `${s} ep ${ep}: the scenes are well-executed; the episode they add up to is less certain about itself.`,
      ...
      (s, ep) => `${s} ep ${ep}: a capable hour with an ending that doesn't land. The show can do this better.`,
      ...
      (s, ep) => `${s} episode ${ep}: the direction is doing work the script hasn't finished. A gap worth watching.`,
      (s, ep) => `Episode ${ep} of ${s}: this show is capable of better. This week is evidence of that, not against it.`,
      (s, ep) => `${s} ep ${ep}: a serviceable hour that doesn't deepen what the season has been building. Watching.`,
      ...
      (s, ep) => `${s} ep ${ep}: competent in parts, soft in others. The aggregate is below where the season has been.`,
    ...
    low: [
      ...
      (s, ep)           => `${s} episode ${ep}: the show is not doing what it set out to do, and this week, it's not even trying to.`,
      (s, ep)           => `Episode ${ep} of ${s} is a failure of craft and intention simultaneously. That's harder to do than it sounds.`,
      (s, ep)           => `${s} ep ${ep}: the writing room lost the thread. This is what that looks like on screen.`,
      ...
      (s, ep)           => `${s} ep ${ep}: an hour that doesn't trust its audience, its story, or itself. All three problems at once.`,
    ...
    trendUp: [
      ...
      (s, ep) => `${s} episode ${ep}: the season isn't lost. This week confirms that. A meaningful correction.`,
      ...
      (s, ep) => `${s} episode ${ep}: the direction was always there. The writing caught up. That's the difference.`,
      ...
      (s, ep) => `${s} episode ${ep}: the show that the first few weeks promised is visible again. About time.`,
    ],
    trendDown: [
      ...
      (s, ep) => `${s} ep ${ep}: the recent quality hasn't held. One week isn't a trend. Two is worth noting.`,
      (s, ep) => `Episode ${ep} of ${s} falls short of last week's mark. The gap is small but the direction is clear.`,
      (s, ep) => `${s} episode ${ep}: what was working last week isn't working here. The execution slipped.`,
      ...
      (s, ep) => `${s} ep ${ep}: the week the season's momentum stalls. May not hold. Currently stalled.`,
      ...
      (s, ep) => `${s} ep ${ep}: the recent quality raised expectations. This episode doesn't meet them. Acknowledged.`,
    ],
  ...
  insider: {
    ...
    high: [
      ...
      (s, ep) => `${s} episode ${ep}: the industry response is running ahead of the audience response. Both are positive. That's unusual. Notable.`,
    ...
    midHigh: [
      ...
      (s, ep) => `${s} ep ${ep}: the internal read is "fine." In context, "fine" is exactly where they need to be.`,
      ...
      (s, ep) => `${s} ep ${ep}: no flags, no panic, no extraordinary enthusiasm. Steady is the word. Steady works mid-season.`,
      (s, ep) => `Internal temperature on ${s} after episode ${ep} is warm. Not hot. Warm. The season is on track.`,
      (s, ep) => `${s} episode ${ep}: people inside are satisfied. Satisfaction is underrated at this point in a season.`,
      (s, ep) => `Tracking ${s} episode ${ep}. The read is consistent with the rest of the season. That's more than some shows manage.`,
      (s, ep) => `${s} ep ${ep}: the kind of episode where the room breathes a little. Good craftsmanship, nothing to fix. Good week.`,
      ...
      (s, ep) => `${s} episode ${ep}: watching and the internal read is positive but not effusive. Mid-season competence. Earned.`,
      (s, ep) => `${s} ep ${ep}: the show is not overextending, not pulling back. Holding line. That's what you want from a healthy season.`,
      ...
      (s, ep) => `${s} ep ${ep}: the number wasn't flashy but the qualitative read is good. The show is landing with the audience it has.`,
    ...
    midLow: [
      ...
      (s, ep) => `${s} ep ${ep}: the internal read is mixed. There are notes. There are always notes, but this week there are more.`,
      ...
      (s, ep) => `${s} ep ${ep}: not the worst read internally, but not clean either. The network will be watching the next one closely.`,
      ...
      (s, ep) => `${s} episode ${ep}: the room is self-aware about where this episode fell short. That self-awareness is at least present.`,
      (s, ep) => `Tracking ${s} ep ${ep}. The read I'm getting: "they know." The show's people are watching the same thing we are.`,
      (s, ep) => `${s} episode ${ep}: the season's weakest week so far, per sources. Still within the zone. Just toward the bottom of it.`,
      (s, ep) => `${s} ep ${ep}: the internal mood is cautious without being panicked. The good run earlier in the season is still the story.`,
      (s, ep) => `Sources flag ${s} episode ${ep} as a soft landing. The show needs a strong week next. The room knows that.`,
      (s, ep) => `${s} episode ${ep}: the read from inside the building is "not our best." That's the honest version. Still watching.`,
      (s, ep) => `The ${s} creative team is aware that episode ${ep} didn't land the way the previous weeks did. Watching the response.`,
      (s, ep) => `${s} ep ${ep}: a creditable effort with a softer result than usual. The network is patient. For now.`,
    ],
    low: [
      ...
      (s, ep) => `${s} ep ${ep}: internally, the read is bad. The creative team knows. The network knows. It's being discussed.`,
      ...
      (s, ep) => `${s} ep ${ep}: the industry isn't writing the show off. But the word "however" is appearing in more sentences about it.`,
      (s, ep) => `The ${s} episode ${ep} conversation has moved from "is it working?" to "what needs to change?" Different conversation.`,
      (s, ep) => `${s} episode ${ep}: the renewal math is getting harder. Sources who were optimistic a month ago are hedging more now.`,
    ...
    trendUp: [
      ...
      (s, ep) => `${s} episode ${ep}: the internal read today is warmer than last week. The show bought itself some goodwill.`,
      (s, ep) => `The ${s} episode ${ep} conversation: "that's more like it." Direct quote from someone inside. That's the week.`,
      (s, ep) => `${s} ep ${ep}: the recovery episode the show needed. Sources say the room feels it too. Timing matters.`,
      ...
      (s, ep) => `${s} ep ${ep}: the bounce-back the network was waiting to see. Not everything, but something. Important something.`,
    ...
    trendDown: [
      ...
      (s, ep) => `${s} ep ${ep}: not last week. Sources noting the drop without escalating the conversation. Yet.`,
      ...
      (s, ep) => `${s} ep ${ep}: a step back after a decent run. The trajectory conversation has restarted inside the building.`,
      (s, ep) => `${s} episode ${ep}: softer than the prior week. The "momentum" word is no longer being used internally.`,
      ...
      (s, ep) => `${s} episode ${ep}: down from last week in ways that are directional. The room is looking at the back half carefully.`,
      (s, ep) => `${s} ep ${ep}: the positive momentum the previous episode generated was not held here. Watching closely.`,
    ],
  ...
  numbers: {
    ...
    high: [
      ...
      (s, ep) => `${s} episode ${ep}: best-performing week this season by the primary metric. The audience grew.`,
      (s, ep) => `The ${s} episode ${ep} number is a statement. Shows that peak this late in a season usually don't drop again.`,
      (s, ep) => `${s} ep ${ep}: the viewership ceiling got raised this week. Worth noting for the season average.`,
      ...
      (s, ep) => `${s} ep ${ep}: strong across both live and delayed. The whole number tells the same story: people want this show.`,
      ...
      (s, ep) => `${s} episode ${ep}: the show's floor keeps rising. That's a fundamentally different tracking story than where it started.`,
    ...
    midHigh: [
      ...
      (s, ep) => `${s} ep ${ep}: the number is where it's been. Consistent is the operative word. Not exciting. Not concerning.`,
      (s, ep) => `${s} episode ${ep} viewership holds flat from last week. In a crowded week, flat is fine.`,
      (s, ep) => `${s} ep ${ep}: a mid-season number that matches the mid-season pattern. The show has found its groove.`,
      (s, ep) => `Viewership on ${s} episode ${ep}: tracking to the season average within a narrow band. Reliable.`,
      (s, ep) => `${s} ep ${ep}: no growth, no decline, same audience every week. That's either a ceiling or a floor. Watching.`,
      (s, ep) => `${s} episode ${ep}: the number doesn't tell a story either direction. The show's audience showed up. Same as last week.`,
      (s, ep) => `${s} ep ${ep}: stable viewership in an unstable time slot environment. That's worth more than the raw number suggests.`,
      ...
      (s, ep) => `${s} ep ${ep}: not the week that grows the audience, not the week that shrinks it. Maintenance viewership. Healthy.`,
      (s, ep) => `${s} episode ${ep}: in the expected band. The show's audience has plateaued and that's not necessarily bad.`,
    ],
    midLow: [
      ...
      (s, ep) => `${s} episode ${ep}: the viewership dipped below the season average. Small dip. Still a dip.`,
      (s, ep) => `${s} ep ${ep} viewership: soft. Not collapsing, but the trend line is bending in the wrong direction.`,
      (s, ep) => `${s} episode ${ep}: the week-over-week decline isn't dramatic, but it's now been consecutive. That's the flag.`,
      (s, ep) => `${s} ep ${ep}: the number came in below where this show has been tracking. The audience thinned this week.`,
      (s, ep) => `${s} episode ${ep}: viewership in the range that gets networks asking questions. Not answering them yet.`,
      (s, ep) => `${s} ep ${ep}: the show is not losing fast. It's losing slow. In some ways that's harder to flag. Worth flagging.`,
      (s, ep) => `${s} episode ${ep}: the audience that stays is loyal. The audience that's been leaving is now visible in the data.`,
      (s, ep) => `${s} ep ${ep}: retention is holding but the sampling side of the number is soft. The show isn't recruiting new viewers.`,
      (s, ep) => `${s} episode ${ep}: the number is fine in isolation. In context of the trend, it's a softer read.`,
      (s, ep) => `${s} ep ${ep}: a viewership week that gives the renewal conversation something to account for.`,
      (s, ep) => `${s} episode ${ep}: not a disaster number. The kind of number that accumulates into a problem if it keeps going.`,
      (s, ep) => `${s} ep ${ep}: the show has been here before this season and bounced. Once more wouldn't be surprising.`,
      (s, ep) => `${s} episode ${ep}: the viewership story this week is "hold." The show held. Below where it should be, but held.`,
      (s, ep) => `${s} ep ${ep}: viewership below the season midpoint average. The numbers are becoming a storyline.`,
      (s, ep) => `${s} episode ${ep}: week-over-week decline small enough to explain away, large enough to appear in the monthly chart.`,
      (s, ep) => `${s} ep ${ep}: not a crisis number. A "this can't keep happening" number. There's a difference. Barely.`,
    ],
    low: [
      ...
      (s, ep) => `${s} ep ${ep}: the viewership slide is no longer deniable. The data says what it says.`,
      (s, ep) => `${s} episode ${ep}: the number came in and the story it tells is not favorable. The audience has thinned significantly.`,
      (s, ep) => `${s} ep ${ep}: the lowest viewership of the season so far. The trend has become a direction.`,
      (s, ep) => `${s} episode ${ep}: the renewal number is now a hard argument to make. The data will be in every meeting.`,
      (s, ep) => `${s} ep ${ep}: viewership at the bottom of what the show has posted. This is the week the math changes.`,
      (s, ep) => `${s} episode ${ep}: the audience has been leaving. This week it showed in the number clearly.`,
      (s, ep) => `${s} ep ${ep}: one of the lower viewership weeks this show has posted. The trajectory is the issue, not just the number.`,
      (s, ep) => `${s} episode ${ep}: the raw viewership number is the renewal conversation now. It's not a comfortable number.`,
      (s, ep) => `${s} ep ${ep}: below what anyone was projecting. The show has a viewership problem that episode ${ep} made harder to argue against.`,
    ],
    finaleHigh: [
    ...
    trendUp: [
      ...
      (s, ep) => `${s} episode ${ep}: week-over-week viewership is up. The slide stopped. That's the news.`,
      (s, ep) => `${s} ep ${ep}: the number climbed back up from last week. Not all the way back, but the direction changed.`,
      (s, ep) => `${s} episode ${ep}: the viewership recovery is real. The audience that was drifting came back this week.`,
      (s, ep) => `${s} ep ${ep}: up from last week. In context of the recent trend, that's a meaningful data point.`,
      (s, ep) => `${s} episode ${ep}: viewership growth week-over-week. Small but real. The trajectory has turned.`,
      (s, ep) => `${s} ep ${ep}: the bounce-back number the network was waiting to see. Doesn't fix everything. Fixes the narrative.`,
      (s, ep) => `${s} episode ${ep}: the viewership uptick is not a sample size issue — it held through the hour. Real recovery.`,
      (s, ep) => `${s} ep ${ep}: the number this week is higher than the recent stretch. The show may have found its floor.`,
      (s, ep) => `${s} episode ${ep} viewership: up. After the recent run, "up" is the number that matters most right now.`,
      (s, ep) => `${s} ep ${ep}: the audience that left in the prior weeks appears to be coming back. Early, but the data supports it.`,
      (s, ep) => `${s} episode ${ep}: the week-over-week increase is modest but consistent. The trend has reversed direction.`,
      (s, ep) => `${s} ep ${ep}: viewership recovery confirmed. The renewal math is different this week than it was last week.`,
    ],
    trendDown: [
      ...
      (s, ep) => `${s} ep ${ep}: the audience erosion that started last week continued this week. The trend line is clear.`,
      (s, ep) => `${s} episode ${ep}: viewership dipped from last week's stronger number. The question is whether it holds or continues.`,
      (s, ep) => `${s} ep ${ep}: week-over-week decline. Second consecutive soft week changes the label from "fluctuation" to "trend."`,
      (s, ep) => `${s} episode ${ep}: the viewership number fell from last week. The show had built momentum. This week spent some.`,
      (s, ep) => `${s} ep ${ep}: the uptick from last week wasn't sustained. The number slid back. Watching carefully.`,
      (s, ep) => `${s} episode ${ep}: down from the prior week. The trajectory arrow points in a direction the renewal math doesn't like.`,
      (s, ep) => `${s} ep ${ep}: viewership declined. The rate of decline is what separates a rough week from a rough season. Still watching.`,
      (s, ep) => `${s} episode ${ep}: the number slipped. The audience that left last week didn't come back this week. That's two weeks.`,
      (s, ep) => `${s} ep ${ep}: not catastrophic viewership. But lower than last week in a week the show needed to hold.`,
      (s, ep) => `${s} episode ${ep}: viewership trending in the wrong direction. The show needs to stabilize before the finale math gets hard.`,
    ],
  ...
  meme: {
    ...
    high: [
      ...
      (s, ep)          => `${s} episode ${ep}: the episode I will use to convert everyone I know into fans. starting tonight.`,
    ...
    midHigh: [
      ...
      (s, ep) => `${s} ep ${ep}: [passing grade meme]. it passed. it gets to keep going.`,
      ...
      (s, ep) => `${s} episode ${ep}: a solid television hour. [person tapping head] not every episode has to wreck me.`,
      (s, ep) => `me finishing ${s} ep ${ep}: good. nice. [clicks next episode]. good.`,
      (s, ep) => `${s} episode ${ep}: would recommend. not urgently. but would recommend. [calm thumbs up]`,
      (s, ep) => `the vibe of ${s} episode ${ep}: [guy giving okay sign]. okay. I'm okay. the show is okay.`,
      (s, ep) => `${s} ep ${ep}: [glass of water emoji]. just like. refreshingly adequate. no complaints.`,
      ...
      (s, ep) => `${s} episode ${ep}: the "I feel calm and that's fine" episode. sometimes you need those.`,
      (s, ep) => `me going into ${s} ep ${ep}: curious / me coming out: satisfied. low drama. high quality. [thumbs up].`,
      (s, ep) => `${s} episode ${ep}: [steady hands meme]. no spiraling today. good episode. good vibes. we move.`,
    ...
    midLow: [
      ...
      (s, ep) => `${s} ep ${ep}: [buffering gif]. I'm loading my opinions and they haven't fully arrived.`,
      (s, ep) => `me finishing ${s} episode ${ep}: so that... happened. [long blink]`,
      (s, ep) => `${s} episode ${ep}: [person looking at something with concern]. it's fine. it's just. fine.`,
      ...
      (s, ep) => `${s} ep ${ep}: [the "this is fine" dog but the room is only slightly on fire]`,
      (s, ep) => `the ${s} episode ${ep} vibe: [politely clapping meme]. thank you. you tried.`,
      (s, ep) => `${s} episode ${ep}: [person making "so-so" hand gesture for 45 minutes]`,
      (s, ep) => `me after ${s} ep ${ep}: hm. [checks phone for something else to do]. hm.`,
      (s, ep) => `${s} episode ${ep}: could have been more. could have been less. was exactly in between. [shrug emoji]`,
      (s, ep) => `the ${s} ep ${ep} discourse is very short and ended quickly and that says a lot`,
      (s, ep) => `${s} episode ${ep}: [person blowing out birthday candles for a birthday nobody remembered]. it's fine.`,
      (s, ep) => `me watching ${s} ep ${ep}: I have watched better versions of this episode from this show. I'm not naming them.`,
      (s, ep) => `${s} ep ${ep}: the group chat emoji for this episode was 😐 and I think that covers it`,
      (s, ep) => `${s} episode ${ep}: it exists and I watched it and my feelings about it are medium [scales emoji]`,
    ],
    low: [
      ...
      (s, ep) => `${s} ep ${ep}: [screenshot of a calendar with "disappointing" written on this week]`,
      (s, ep) => `the ${s} writers after episode ${ep}: "we think it went well." / the viewers: [long stare]`,
      (s, ep) => `${s} episode ${ep}: [kid looking disappointed at birthday cake meme]. this is not what I asked for.`,
      (s, ep) => `me after ${s} ep ${ep}: I watched the whole thing. I watched it. I have questions. not good ones.`,
      (s, ep) => `${s} ep ${ep}: [person holding their head in their hands at a computer]`,
      (s, ep) => `the ${s} episode ${ep} group chat: 💀. no further comments.`,
      (s, ep) => `${s} ep ${ep}: the episode that finally made someone in the group chat suggest "maybe take a break from this show"`,
    ],
    finaleHigh: [
    ...
    trendUp: [
      ...
      (s, ep) => `${s} ep ${ep}: [main character walking in slow motion]. the show is back and it knows it.`,
      ...
      (s, ep) => `${s} ep ${ep}: last week was rough. this week? THIS week. [chef's kiss]`,
      ...
      (s, ep) => `${s} ep ${ep}: [getting back up after a fall meme]. the show is on its feet. this is the episode.`,
      ...
      (s, ep) => `${s} ep ${ep}: [the comeback kid meme]. last week was a stumble. this week is the story.`,
    ...
    trendDown: [
      ...
      (s, ep) => `${s} ep ${ep}: [rollercoaster going down very slowly]. we're descending. it's slow. it's happening.`,
      ...
      (s, ep) => `${s} episode ${ep}: [person turning a light dimmer down very slightly]. not dark. just dimmer.`,
      (s, ep) => `me after ${s} ep ${ep}: [waves hand in so-so gesture] it was fine. last week was fine AND exciting.`,
      (s, ep) => `${s} ep ${ep}: the dip you notice but don't mention out loud and then everyone mentions it`,
      (s, ep) => `${s} episode ${ep}: [mild disappointment face]. this is fine. last week was also fine but differently.`,
      (s, ep) => `the ${s} ep ${ep} group chat energy has dropped by about 40% from last week and I think we all feel that`,
      (s, ep) => `${s} ep ${ep}: [person slowly sitting back down after they got up excitedly last week]`,
      (s, ep) => `${s} episode ${ep}: the show that peaked last week is taking a breather this week. I see you. [cautious side eye]`,
      (s, ep) => `${s} ep ${ep}: the decline meme that's too accurate but also not dramatic enough to post without context`,
  ...
  hatewatcher: {
    ...
    high: [
      ...
      (s, ep) => `${s} ep ${ep}: I've been logging criticisms for weeks and this episode took all of them away. furious.`,
      ...
      (s, ep) => `${s} ep ${ep}: the show made me care about something I said I didn't care about. unforgivable. I'll be watching next week.`,
      ...
      (s, ep) => `${s} ep ${ep}: I started watching this show to complain and I am now deeply emotionally invested. I'm the problem.`,
    ...
    midHigh: [
      ...
      (s, ep) => `${s} ep ${ep}: the show is being exactly the show I expected it to be. I keep watching. That's the deal.`,
      (s, ep) => `${s} episode ${ep}: still here. still fine. the reasons I started watching this have not materialized. I remain. 👀🍿`,
      (s, ep) => `${s} ep ${ep} is fine in the way that makes me suspicious. something is going to go wrong. I'm waiting.`,
      (s, ep) => `${s} episode ${ep}: okay. it's okay. I've made peace with "okay." I'm watching an okay show okay.`,
      (s, ep) => `${s} ep ${ep}: fine. consistently, reliably fine. I can't even be interesting about it. it's just fine.`,
      (s, ep) => `me finishing ${s} episode ${ep}: [already opening the next episode] I'm the problem. I know I'm the problem.`,
      (s, ep) => `${s} episode ${ep}: the show that refuses to be bad enough to quit or good enough to get excited. I'm captive.`,
      (s, ep) => `${s} ep ${ep}: decent. adequately decent. I watch it every week and I will continue to watch it every week. 👀🍿`,
      (s, ep) => `${s} episode ${ep} and I've run out of specific complaints. it's just... it. the show. doing the show.`,
      (s, ep) => `${s} ep ${ep}: I came for hate-watching, I'm getting decent-watching. I've adjusted my expectations. Here I am.`,
      (s, ep) => `${s} episode ${ep}: a solid mid that doesn't offend and doesn't delight. I've watched worse. I have opinions on it. 👀🍿`,
      (s, ep) => `${s} ep ${ep}: fine TV. I've accepted that I will watch fine TV until this show gives me a reason to stop. It hasn't.`,
      (s, ep) => `${s} episode ${ep}: look. I keep watching. That's my review. I keep watching.`,
      (s, ep) => `${s} ep ${ep} is adequate and I'm going to be honest with myself about the fact that "adequate" is keeping me here.`,
      (s, ep) => `${s} episode ${ep}: the show is doing a thing and I watched it and I have no strong feelings and I HATE THAT.`,
      (s, ep) => `${s} ep ${ep}: committed to being fine. I respect the commitment to fine even as I resent the fine.`,
    ],
    midLow: [
      ...
      (s, ep) => `${s} ep ${ep}: the cracks I predicted are appearing. I don't feel good about being right.`,
      (s, ep) => `${s} episode ${ep}: this is the episode where I start to feel vindicated and I didn't want to feel vindicated. 👀🍿`,
      (s, ep) => `${s} ep ${ep}: softer than the recent run. I take no pleasure in noting this. I take some. A little.`,
      (s, ep) => `${s} episode ${ep}: the show is starting to show me what I thought I'd be seeing all along. interesting timing.`,
      (s, ep) => `${s} ep ${ep}: I've been keeping notes. Episode ${ep} is going in the "I told you so" column. Reluctantly. 👀🍿`,
      (s, ep) => `${s} episode ${ep}: the wheels are wobbling. I can see the wheels wobbling. I'm still in the car.`,
      (s, ep) => `${s} ep ${ep}: I keep watching because I want to be wrong about where this is going. Still watching. Still not sure.`,
      (s, ep) => `${s} episode ${ep}: the show is drifting in a direction I predicted and I would like to be wrong. Episode ${ep}: I'm not wrong.`,
      (s, ep) => `${s} ep ${ep}: a step in the direction of what I thought this show would always be. unfortunate. still here. 👀🍿`,
      (s, ep) => `${s} episode ${ep}: I remain. I will keep remaining. This is what I signed up for and I signed up for it. I am aware.`,
      (s, ep) => `${s} ep ${ep}: soft. softening. I've been watching it soften. This is not a good feeling. I haven't left.`,
      (s, ep) => `${s} episode ${ep}: I wanted to write "it's fine" and I had to write "it's okay, I guess." Different words. Different week.`,
      (s, ep) => `${s} ep ${ep}: the show did the thing I thought it was going to do and I am processing that. Still watching.`,
      (s, ep) => `${s} episode ${ep}: not terrible enough to quit, not good enough to forgive. the exact space I've occupied all season. 👀🍿`,
      (s, ep) => `${s} ep ${ep}: a show choosing to be this when it could be more. I resent that choice. I watched it anyway.`,
      (s, ep) => `${s} episode ${ep}: below where it was, closer to where I thought it was going. I hate being right. I'm right. 👀🍿`,
    ],
    low: [
      ...
      (s, ep)           => `${s} episode ${ep}: a show happening. to viewers. against their will. (I chose this.) 👀🍿`,
      (s, ep)           => `${s} ep ${ep}: I was right. I hate being right about this specifically. I'm right. It's bad.`,
      (s, ep)           => `${s} episode ${ep} is as bad as I predicted and I got no enjoyment from the prediction. Still watching. 👀🍿`,
      (s, ep)           => `${s} ep ${ep}: this is the version of this show I came in expecting. I didn't want to be correct. I was correct.`,
      (s, ep)           => `${s} episode ${ep}: the mask is off and what's under it is exactly what I suspected. Unpleasant confirmation. Still here.`,
      (s, ep)           => `${s} ep ${ep} is where the show became what I said it would be and I feel nothing about being right except tired.`,
      (s, ep)           => `${s} episode ${ep}: I've been hate-watching. This is the week it stops being fun. Still watching. Different energy. 👀🍿`,
      (s, ep)           => `${s} ep ${ep}: the show chose this. chose this specific thing. I am watching the choice play out. Grimly.`,
      (s, ep)           => `${s} episode ${ep}: I have watched better and worse TV. This week, ${s} is the worse. I will note it and move on.`,
    ],
    finaleHigh: [
    ...
    trendUp: [
      ...
      (s, ep) => `${s} ep ${ep}: the improvement I was hoping wasn't coming is here. I don't know how to feel about being wrong.`,
      ...
      (s, ep) => `${s} episode ${ep}: stronger than last week. I can't argue with it. I tried. It's stronger. 👀🍿`,
      (s, ep) => `${s} ep ${ep}: the bounce I predicted wouldn't come. I was wrong about this one specifically. Acknowledged.`,
      (s, ep) => `${s} episode ${ep} got better and I am updating my notes accordingly. the dip last week may have been the dip.`,
      (s, ep) => `${s} ep ${ep}: better. measurably better. I came to hate-watch and it gave me something better to do.`,
      (s, ep) => `${s} episode ${ep}: the uptick I didn't expect. I respect it. Grudgingly. 👀🍿`,
      (s, ep) => `${s} ep ${ep}: I was ready to write "still declining" and I can't write that. It went up. I'm noting it.`,
      (s, ep) => `${s} episode ${ep}: step back up from last week. I hate when the show does the thing I hoped it wouldn't do (be good).`,
      (s, ep) => `${s} ep ${ep}: better than the week before in ways I didn't predict. My predictions are on record. Update them.`,
    ...
    trendDown: [
      ...
      (s, ep) => `${s} episode ${ep}: the dip I called is happening. I take no pleasure in this. Some pleasure. A little.`,
      (s, ep) => `${s} ep ${ep}: below last week. the direction I predicted. I'm keeping notes. The notes are correct. 👀🍿`,
      (s, ep) => `${s} episode ${ep}: down from the prior week. I noted last week that it might slip. It slipped. Filed.`,
      (s, ep) => `${s} ep ${ep}: I've been tracking this and the track is going down. Week ${ep}. Logged.`,
      (s, ep) => `${s} episode ${ep}: I said last week there was something to watch. I was watching. I was right. 👀🍿`,
      (s, ep) => `${s} ep ${ep}: softer than last week. I've been saying this was coming. It came. I feel like a very tired prophet.`,
      (s, ep) => `${s} episode ${ep}: last week was better. the week before was better. I'm tracking this so someone has to.`,
      (s, ep) => `${s} ep ${ep}: a continued slide from last week. I am watching. That's my thing. I watch and note. Still watching. 👀🍿`,
      (s, ep) => `${s} episode ${ep}: the trajectory is not what I want it to be. I want to be wrong about where it's going.`,
      (s, ep) => `${s} ep ${ep}: the decline continues. Two weeks. That's a trend. I've been here for both weeks. Noted.`,
      (s, ep) => `${s} episode ${ep}: weaker than last week. I've been calling this since the season started and I hate being here.`,
      (s, ep) => `${s} ep ${ep}: slipped from the prior week. the hate-watch is becoming a different kind of uncomfortable. 👀🍿`,
    ],
  ...
  recapper: {
    ...
    high: [
      ...
      (s, ep)          => `${s} episode ${ep}: I've been covering TV long enough to recognize when a show is doing something exceptional. This week: exceptional.`,
      (s, ep)          => `The writing in ${s} episode ${ep} is doing three things at once and landing all three. That doesn't happen every week. 📺`,
      (s, ep)          => `${s} episode ${ep}: the kind of hour that makes this job feel worth doing. Detailed recap coming tonight.`,
      ...
      (s, ep)          => `${s} episode ${ep}: a complete hour — beginning, middle, end, each earning the next. Recap up tonight.`,
      (s, ep)          => `${s} ep ${ep} is why you don't give up on shows mid-season. It's building toward something and this week it arrived.`,
      (s, ep)          => `${s} episode ${ep}: one of those weeks where the recap writes itself because the episode did everything. 📺`,
    ],
    midHigh: [
      ...
      (s, ep) => `${s} episode ${ep}: two strong scenes, solid connective tissue, a story that moved forward. Good week. Recap tonight.`,
      (s, ep) => `${s} ep ${ep}: if this is the floor, that's a solid floor. Recap up with the longer take. 📺`,
      (s, ep) => `${s} episode ${ep} has what mid-season episodes need: momentum, character work, no wasted minutes. Covered it. 📺`,
      (s, ep) => `${s} ep ${ep}: not the episode I'll mention in the season review, but a reason the season review will be positive.`,
      ...
      (s, ep) => `${s} episode ${ep}: the show running at its mid-range, which for this show is above the competition's high range. 📺`,
      (s, ep) => `${s} ep ${ep}: a week where the show does everything right without doing anything new. That's fine. Recap up. 📺`,
      (s, ep) => `${s} episode ${ep}: I'd recommend this episode to someone on the fence about the show. It's the show at its most reliable.`,
      (s, ep) => `${s} ep ${ep}: the season's workhorse episode. Someone has to write the bridge hour well. They did. 📺`,
      (s, ep) => `${s} episode ${ep}: the recap is easier to write when the episode is this consistently made. Good week. 📺`,
      (s, ep) => `${s} ep ${ep}: nothing to write home about, except that the craftwork is good and the story advanced and the hour passed well.`,
      (s, ep) => `${s} episode ${ep}: I'm writing the recap with genuine warmth. A quietly good episode. Recap up tonight. 📺`,
      (s, ep) => `${s} ep ${ep}: the show is in control of its material this week. Nothing flashy. Everything functional. That's enough.`,
    ...
    midLow: [
      ...
      (s, ep) => `${s} episode ${ep}: the concept is there; the execution lost something between the page and the screen.`,
      (s, ep) => `${s} ep ${ep}: a week where the show's weaknesses are more visible than its strengths. Recap up with specific notes. 📺`,
      (s, ep) => `${s} episode ${ep}: the pacing was off this week in ways that the recap is going to have to explain carefully.`,
      (s, ep) => `${s} ep ${ep}: I've given this show more benefit of the doubt than most. This week it asked for a lot. 📺`,
      (s, ep) => `Recapping ${s} ep ${ep}: the hour has two good moments and a lot of space between them.`,
      (s, ep) => `${s} episode ${ep}: the show's ambition is still there. The follow-through is where this week got soft. 📺`,
      (s, ep) => `${s} ep ${ep}: a mid-season dip that I hope is temporary. The recap is up and it's a mixed bag.`,
      (s, ep) => `${s} episode ${ep}: when a show has earned goodwill, a soft week is forgivable. This is a soft week. Recap up. 📺`,
      (s, ep) => `${s} ep ${ep}: the writers set up something interesting and the episode didn't fully cash it in. Notes in the recap.`,
      (s, ep) => `${s} episode ${ep}: watching with genuine investment makes this week harder to be mild about. Recap tonight. 📺`,
      (s, ep) => `${s} ep ${ep}: the show can do better — we've seen it do better — and this week it didn't. Honest recap up.`,
      (s, ep) => `${s} episode ${ep}: the elements are right. The assembly is off. The recap is going to require some nuance. 📺`,
      (s, ep) => `${s} ep ${ep}: a frustrating hour from a show I like, which is a specific kind of frustrating.`,
      (s, ep) => `${s} episode ${ep}: not the week to start watching, not the week that makes you quit. The middle-ground week. 📺`,
      (s, ep) => `${s} ep ${ep}: the recap exists and it's honest. A notch below where this show has been. Said with care.`,
    ],
    low: [
      ...
      (s, ep) => `${s} episode ${ep}: the season has been uneven and this is the week the uneven fully shows. Recap up. 📺`,
      (s, ep) => `${s} ep ${ep}: the honest recap is up and I don't love what I wrote. The episode earned it.`,
      (s, ep) => `${s} episode ${ep}: the hardest recaps to write are the ones where you want the show to be better than the episode. 📺`,
      (s, ep) => `${s} ep ${ep}: a bad week deserves an honest recap. Recap is up. I covered it fairly. I don't enjoy saying this.`,
      (s, ep) => `${s} episode ${ep}: the structural issues that have been building this season arrived this week. Addressed in the recap. 📺`,
      (s, ep) => `${s} ep ${ep}: I've been covering this show with genuine enthusiasm. This week my enthusiasm is working overtime. 📺`,
      (s, ep) => `${s} episode ${ep}: not the recap I wanted to write tonight. The show asked me to. Recap is up.`,
      (s, ep) => `${s} ep ${ep}: the low point of the season so far. Saying that as someone who wants the show to succeed. 📺`,
      (s, ep) => `${s} episode ${ep}: a rough week. Honest recap up. The show is better than this. This week it wasn't. 📺`,
      (s, ep) => `${s} ep ${ep}: the episode that tests whether the goodwill built earlier in the season was durable. Recap tonight.`,
    ],
    finaleHigh: [
    ...
    trendUp: [
      ...
      (s, ep) => `${s} ep ${ep}: the bounce I was hoping for. Better than last week in the ways that matter. Recap tonight. 📺`,
      (s, ep) => `${s} episode ${ep}: after last week, I'm relieved. The show found its footing again. Detailed recap up. 📺`,
      (s, ep) => `${s} ep ${ep}: a meaningful step forward from the prior week. The problems last week are addressed this week.`,
      (s, ep) => `${s} episode ${ep}: the recovery episode done right. Not just okay — genuinely better. Recap coming tonight. 📺`,
      (s, ep) => `${s} ep ${ep}: the week the season earns back the credibility last week spent. Covered it tonight. 📺`,
      (s, ep) => `${s} episode ${ep}: a resurgence week. The show remembered what it's doing and why. Recap up. 📺`,
      (s, ep) => `${s} ep ${ep} is the answer to last week's question. The show can still do it. Recap up.`,
      (s, ep) => `${s} episode ${ep}: the kind of week where recapping is a pleasure again. Solid bounce-back. Up tonight. 📺`,
      (s, ep) => `${s} ep ${ep}: a step back up after a soft patch. The season's trajectory just changed. Noted in the recap. 📺`,
      (s, ep) => `${s} episode ${ep}: I've been waiting for the show to do this. It did it. Recap up and I'm not unhappy writing it. 📺`,
      (s, ep) => `${s} ep ${ep}: the season just got more interesting. The recovery episode is here and it's a good one. 📺`,
      (s, ep) => `${s} episode ${ep}: credit where it's due — a meaningful improvement after a rough stretch. Honest recap up. 📺`,
    ],
    trendDown: [
      ...
      (s, ep) => `${s} ep ${ep}: a step back from last week in ways the recap is going to have to document carefully. 📺`,
      (s, ep) => `${s} episode ${ep}: the uneven week after a solid week. The pattern is becoming harder to ignore. Recap tonight.`,
      (s, ep) => `${s} ep ${ep}: not the follow-up the season needed. The recap tonight reflects that honestly. 📺`,
      (s, ep) => `${s} episode ${ep}: last week set the bar and this week didn't clear it. The gap is specific and it's in the recap. 📺`,
      (s, ep) => `${s} ep ${ep}: a softer episode after a stronger one. The show has more in it than this week showed. Recap up.`,
      (s, ep) => `${s} episode ${ep}: the momentum from last week hasn't carried. Notes on why in tonight's recap. 📺`,
      (s, ep) => `${s} ep ${ep}: the back half of this season is going to need to answer for the recent stretch. Recap tonight. 📺`,
      (s, ep) => `${s} episode ${ep}: below last week and below where the season has been tracking. Honest recap up. 📺`,
      (s, ep) => `${s} ep ${ep}: the dip week. Every season has one. This is this season's. Covering it honestly tonight.`,
      (s, ep) => `${s} episode ${ep}: softer than the prior episode in ways that matter for the season as a whole. Recap up. 📺`,
      (s, ep) => `${s} ep ${ep}: a slip after a solid run. The show was building something. This week it paused the build. 📺`,
      (s, ep) => `${s} episode ${ep}: I'm covering this honestly and the honest version is that last week was better. Recap tonight.`,
    ],
  ...
  parasocial: {
    ...
    high: [
      ...
      (s, ep)          => `${s} ep ${ep}: they made me care and then they did THAT and I need to take a walk. I won't though. I'll rewatch it.`,
      (s, ep)          => `${s} episode ${ep}: the episode where I admitted to myself that this show has changed me a little. it has.`,
    ...
    midHigh: [
      ...
      (s, ep) => `${s} episode ${ep}: good episode. I loved the cast in it. as I love the cast in all episodes. that's the thing about love.`,
      (s, ep) => `${s} ep ${ep}: not the one that breaks me but one that keeps me coming back. that's a different kind of essential.`,
      (s, ep) => `honestly ${s} ep ${ep} gave me exactly what I needed from it. a good hour with characters I love. that's enough.`,
      (s, ep) => `${s} episode ${ep}: I'm satisfied. a satisfied watch is underrated. this cast continues to have my heart.`,
      ...
      (s, ep) => `${s} ep ${ep}: I smiled a lot. I didn't cry. I love this show. this is a calm love and it feels good.`,
      (s, ep) => `${s} episode ${ep}: solid and I mean that warmly. the cast showed up. they always show up. I love them for it.`,
      (s, ep) => `not my favorite ${s} episode but ep ${ep} reminded me why I got into this in the first place. the cast, always the cast.`,
      (s, ep) => `${s} ep ${ep}: a week where the show did exactly what I needed. I needed a good hour. I got a good hour.`,
      (s, ep) => `${s} episode ${ep}: I am emotionally intact. the show is good. the cast is perfect. I'm fine. I'm great, actually.`,
      (s, ep) => `${s} ep ${ep}: a safe harbor episode. not the standout of the season. a show I love being a show I love. that's plenty.`,
      (s, ep) => `${s} episode ${ep} gave me what I come here for: this cast, doing their thing, doing it well. I'm at peace with ep ${ep}.`,
      (s, ep) => `${s} ep ${ep}: really good. I cared about the characters. I always care about the characters. it was a good week.`,
      (s, ep) => `${s} episode ${ep}: the episode I'd show someone to explain why I'm invested. it's just. good. they're good. I love them.`,
      (s, ep) => `${s} ep ${ep}: the cast continues to absolutely deliver. that's what I show up for. they kept showing up. I'm happy.`,
    ],
    midLow: [
      ...
      (s, ep) => `${s} ep ${ep}: not the week I needed but I trust this cast and this cast is still the reason I'm here.`,
      (s, ep) => `I watch ${s} for this cast and this cast was there in episode ${ep}. the episode could've been better. the cast: always them.`,
      (s, ep) => `${s} ep ${ep}: soft week for the show. not a soft week for my attachment to the characters. that part is permanent.`,
      ...
      (s, ep) => `${s} ep ${ep}: the cast is doing everything they can with the material and I love them for it. the material was softer this week.`,
      (s, ep) => `${s} episode ${ep}: I've been here since the premiere. I will be here next week. this week was just not the best week.`,
      ...
      (s, ep) => `${s} episode ${ep}: I trust this show and this cast with my feelings and this week they required some patience. still here.`,
      (s, ep) => `${s} ep ${ep}: a softer week that required me to lean on my history with the show more than this week's episode.`,
      (s, ep) => `${s} episode ${ep}: this is the week you have to decide if you're in it or you're not. I decided a long time ago. I'm in it.`,
      (s, ep) => `${s} ep ${ep}: not every episode is a favorite. this is not a favorite. I am still a fan. the distinction matters.`,
      (s, ep) => `the ${s} cast continues to give everything in episode ${ep} and I love them for giving everything to softer material.`,
      (s, ep) => `${s} episode ${ep}: the week where I'm conscious of my loyalty rather than just living inside it. still loyal. just conscious.`,
      (s, ep) => `${s} ep ${ep}: a week that asks more of the fan than the show delivers. I gave more. I always give more. this cast.`,
    ],
    low: [
      ...
      (s, ep) => `${s} episode ${ep}: I love this cast more than I love most things. this episode made that love do some heavy lifting.`,
      (s, ep) => `${s} ep ${ep}: the bad week. every show has one. this show's bad week arrived. I'm still here. I'm hurt, but I'm here.`,
      (s, ep) => `I've been loyal to ${s} through a lot of things and episode ${ep} is the thing that is testing what "loyal" means.`,
      (s, ep) => `${s} episode ${ep}: I chose to care about this show and this cast and they gave me episode ${ep} and I'm dealing with that.`,
      (s, ep) => `${s} ep ${ep}: the writers room let the cast down this week and the cast is giving everything and I love them anyway.`,
      (s, ep) => `${s} episode ${ep} is the kind of disappointment you only feel when you love something. I love it. I'm disappointed.`,
      (s, ep) => `${s} ep ${ep}: I'm not leaving. I want to say that clearly. I'm not leaving. But. Episode ${ep}. I have notes.`,
      (s, ep) => `I cannot believe ${s} made me feel this way about episode ${ep}. not the feeling I came here for. I'm still here though.`,
      (s, ep) => `${s} ep ${ep}: this cast deserved better material this week. I needed to say that. I love them. they deserved better.`,
    ],
    finaleHigh: [
    ...
    trendUp: [
      ...
      (s, ep) => `${s} ep ${ep}: the bounce-back I needed. the cast delivered. they always deliver when the material lets them.`,
      ...
      (s, ep) => `${s} episode ${ep}: the recovery I was waiting for. the cast is doing the thing they do when the writing lets them.`,
      (s, ep) => `I held on through the rough patch and ${s} ep ${ep} is the reason you hold on. I held. the show delivered. good trade.`,
      (s, ep) => `${s} ep ${ep}: better. measurably, visibly better. the cast is RUNNING and I am running with them.`,
      (s, ep) => `${s} episode ${ep} bounced back and I'm pretending I was calm about this the whole time. I wasn't. I am now.`,
      (s, ep) => `${s} ep ${ep}: the return. I was always here. the show came back to me. we are reunited. I'm not crying. I'm crying a little.`,
      (s, ep) => `${s} episode ${ep}: the improvement I needed to see. the cast grabbed this material and ran with it. I love them so much.`,
      (s, ep) => `${s} ep ${ep}: back on track and my parasocial attachment has been renewed for another season. (I already renewed it.)`,
    ...
    trendDown: [
      ...
      (s, ep) => `${s} episode ${ep}: not as good as last week and I'm doing my best not to spiral. doing okay. mostly.`,
      (s, ep) => `I need ${s} to understand that ep ${ep} did not hit as hard as last week and I noticed and I care and I'm still here.`,
      (s, ep) => `${s} ep ${ep}: last week had me SO excited and this week was good but not as good and I am processing that.`,
      (s, ep) => `${s} episode ${ep} slipped from last week and I'm telling myself it's one episode. it is one episode. it better be.`,
      (s, ep) => `${s} ep ${ep}: I love this cast and this cast was great and the episode was softer than last week and those are both true.`,
      (s, ep) => `${s} episode ${ep}: a small dip from the week before and I'm cataloguing it and I'm still fully committed. always.`,
      (s, ep) => `${s} ep ${ep}: I came in riding the high from last week and I'm leaving at a slightly lower altitude. I'm okay. the show is okay.`,
      ...
      (s, ep) => `${s} ep ${ep}: the dip after the good week. I feel it. I'm not going anywhere. I just. feel it.`,
      (s, ep) => `${s} episode ${ep}: last week built something and this week didn't quite continue it. I trust the show. I trust this cast.`,
      (s, ep) => `${s} ep ${ep}: a small step back from last week and I'm trying not to give that too much weight. giving it a little weight.`,
      (s, ep) => `${s} episode ${ep}: not as strong as last week. I say that as someone who will be here regardless. I'll be here regardless.`,
    ],
